...
print("🔢 REAL-TIME ASL NUMBER DETECTOR")
print("Loading number model...")

# Load number model
try:
    model = keras.models.load_model('models/number_model.h5')
    with open('models/number_label_encoder.pkl', 'rb') as f:
        label_encoder = pickle.load(f)
    
    print("✅ Number model loaded!")
    print(f"🎯 Can detect: 0 to 9 ({len(label_encoder.classes_)} numbers)")
...
print("\n🚀 READY! Show ASL numbers (0-9) to the camera!")
print("📝 Instructions:")
...
prediction_history = []
current_number = "Show Hand"
confidence = 0.0
target_number = "0"  # You can change this for your lessons
...
while True:
    ...
    if results.multi_hand_landmarks:
        for hand_landmarks in results.multi_hand_landmarks:
            ...
            predicted_number = label_encoder.inverse_transform([predicted_idx])[0]
            ...
            if current_confidence > 0.6:
                prediction_history.append(predicted_number)
            ...
            cv2.putText(frame, f"TARGET: {target_number}", (20, 40), 
                       cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 255, 255), 2)
            
            cv2.putText(frame, f"YOUR SIGN: {current_number}", (20, 80), 
            ...
            if current_number == target_number and confidence > 0.8:
    ...
    cv2.putText(frame, "Show ASL numbers: 0 to 9", (20, frame.shape[0] - 80), 
               cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 255, 255), 2)
    cv2.putText(frame, "Press 'Q' to quit | 'C' to clear", (20, frame.shape[0] - 40), 
               cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 255, 255), 2)
    
    # Display the frame
    cv2.imshow('ASL Number Teacher - SignXpress', frame)
...
hands.close()
print("✅ Number detector closed successfully!")